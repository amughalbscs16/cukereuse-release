...
\title{\tool: Static, Paraphrase-Robust Duplicate Step Detection for Cucumber/Gherkin, with an Empirical Corpus and a Cognitive Dimensions Analysis}
...
\begin{abstract}
Behaviour-Driven Development (BDD) suites in long-lived projects accumulate
step-text duplication whose maintenance cost is established in prior
BDD-quality work
\citep{binamungu2018vst,oliveira2017quality,irshad2021jss}. Existing
detection techniques rely on dynamic execution tracing
\citep{binamungu2018vst,binamungu2018saner,binamungu2020xp,binamungu2020phd,binamungu2023jss}
or normalised compression distance at industrial scale
\citep{irshad2020ease,irshad2021jss,irshad2022ist}, leaving a gap: a
purely \emph{static}, \emph{paraphrase-robust}, \emph{step-level}
duplicate detector usable on any repository without a configured
test runner. We fill that gap with \tool, an open-source Python CLI
combining exact hashing, Levenshtein ratio, and sentence-transformer
embeddings \citep{reimers2019sbert} in a layered pipeline, released
alongside an empirical corpus of 347 public GitHub repositories,
approximately 23.7k parsed \texttt{.feature} files and 1.11M Gherkin
steps. The step-weighted exact-duplicate rate is 80.2\,\%; the
median-repository rate is 52.5\,\%, with Spearman $\rho = 0.51$
between repository size and duplication rate, so the pooled
headline is pulled up by a minority of very large suites. Under
paraphrase-aware hybrid clustering the top cluster
(\texttt{the response status is 200 OK}) groups 20.7k occurrences
across 2.2k distinct files compared to 6.1k under exact string-match.
Against 1{,}020 author-labelled pairs released with a written rubric,
we report precision, recall, and F$_1$ per strategy with bootstrap
95\,\% confidence intervals under two evaluation protocols: the
primary rubric and a score-free second-pass relabelling whose
positive rules consult no cosine or Levenshtein values. The strongest
pair-level result is near-exact (Levenshtein) detection at
F$_1 = 0.822$ on the score-free labels; the semantic detector reaches
F$_1 = 0.906$ [0.886, 0.923] under the primary rubric. Two lexical baselines standing in for
classical token-stream clone detectors (SourcererCC-style,
NiCad-style) reach primary F$_1 = 0.761$ and $0.799$. The paper
also contains, to our knowledge, the first Cognitive Dimensions of
Notations analysis of Gherkin in the peer-reviewed literature
\citep{green1989cdn,green1996petre,blackwell2003cdn}, an author
assessment grounded in corpus evidence; eight of fourteen dimensions
are rated problematic or unsupported. The tool, corpus manifest,
labelled pairs, rubric, and analysis pipeline are released under
permissive licences.
\end{abstract}

\vspace{1ex}
\noindent\textbf{Keywords:} Behaviour-Driven Development; Cucumber; Gherkin;
duplicate detection; clone detection; sentence-transformer; empirical
software engineering; cognitive dimensions of notations; software
maintenance; test specification.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item A Cognitive Dimensions of Notations (CDN) analysis of Gherkin,
        the first in the peer-reviewed literature to our knowledge,
        grounding each of the 14 dimensions in concrete examples drawn
        from the corpus. The ratings are the author's assessment
        (§\ref{sec:cdn} discloses the methodology).
...
\texttt{When the user submits valid credentials}, etc.) is matched at
runtime against a \emph{step definition} in the project's glue code,
typically via regular expression. This matching is the seam at which
...
memory footprint is under 100~MB. This performance envelope makes it
tractable to embed every unique step in our 1.1-million-step corpus on
a consumer laptop in under ten minutes.
...
Table~\ref{tab:corpus-compare} positions our corpus against the three
most directly-comparable prior BDD-quality corpora in published literature.
To our knowledge no prior cross-organisational, cross-repository
\texttt{.feature}-file corpus has been released with a permissive-licensed
pointer bundle and rehydration tooling.

\begin{table}[t]
\centering
\caption{BDD-quality corpora reported in published prior work, compared
to the present corpus. Counts are as reported by the cited authors.
"Cross-org" is yes if the corpus spans more than one independent
organisation or repository owner.}
\label{tab:corpus-compare}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lrlcc}
\toprule
Corpus                                       & Projects & Steps / Scenarios               & Cross-org & Released \\
\midrule
Binamungu et al.\ \citep{binamungu2023jss}   &        4 & 277 scenarios                   & no        & no \\
Irshad et al.\ \citep{irshad2021jss}         &        1 & $\sim$15k steps (internal)      & no        & no \\
Suan \citep{suan2015msc}                     &        3 & few hundred                     & no        & no \\
\textbf{\tool\ (this paper)}                 & \textbf{347} & \textbf{1{,}113{,}616 steps}  & \textbf{yes} & \textbf{yes} \\
\bottomrule
\end{tabular}
\end{table}
...
is the bandwidth optimisation that makes the pipeline tractable on
a consumer laptop.

\section{A Cognitive Dimensions Analysis of Gherkin (author assessment)}
\label{sec:cdn}
...
corpus this pass completes in under one minute on consumer CPU.
...
budget on consumer CPUs (under ten minutes over the 1.1M-step corpus).
...
analysis, output) run end-to-end in approximately three hours
of wall-clock time on a consumer laptop; each stage caches its
...
\(\geq 100\) collapses the sample to 36 repositories,
...
that retains a four-digit repository count while discarding
obvious clone-spam; the cognate filter in Munaiah et al.
...
We sample 1{,}020 pairs of step texts from the corpus, stratified in
170-pair bins across six cosine-similarity bands: \([0.50, 0.70)\),
\([0.70, 0.80)\), \([0.80, 0.85)\), \([0.85, 0.90)\),
\([0.90, 0.95)\), and \([0.95, 1.00)\). Pairs are drawn via streaming
cosine enumeration over a 20{,}000-text random pool of the corpus's
unique texts; low-cosine bands are augmented from random sampling
(where high-cosine pairs are too rare to sample uniformly). For each
pair we record the cosine similarity, the normalised Levenshtein
ratio, and a binary label (duplicate vs.\ not-duplicate) reflecting
whether a BDD maintainer would plausibly want the two phrasings
consolidated into a single step definition.

Labels are produced by the first author under a written rubric
(\texttt{corpus/LABELING\_RUBRIC.md}) that specifies ten ordered
decision rules: R1 parametric variants $\to$ duplicate, R2
synonym-level paraphrase $\to$ duplicate, R3 structural swap $\to$
duplicate, R4 different framework keywords with distinct semantics
($\texttt{call}$ vs $\texttt{callonce}$, $\texttt{def}$ vs
$\texttt{set}$) $\to$ not-duplicate, R5 different HTTP verbs $\to$
not-duplicate, R6 opposite-polarity assertions $\to$ not-duplicate,
R7 presence vs content assertions $\to$ not-duplicate, R8 action vs
assertion $\to$ not-duplicate, R9 outline-placeholder rename $\to$
duplicate, R10 default $\to$ not-duplicate. Every row of
\texttt{labeled\_pairs.jsonl} carries the rule that fired for that
pair, allowing per-pair audit of rubric application. The rubric
is released with the corpus.

Of the 1{,}020 pairs, 494 are labelled as duplicates and 526 as
not-duplicates (48.4\,\% positive rate). Each label carries the
rubric clause that fired, recorded in \texttt{labeled\_pairs.jsonl},
so rubric application is auditable pair by pair. The author reviewed
and verified every label against the underlying step-text pair, and
\texttt{labeled\_pairs.jsonl} is released as part of the artefact. To test the labels' stability under an alternative
decision procedure, the entire 1{,}020-pair set is relabelled under
a score-free second-pass protocol
(Section~\ref{sec:evaluation:score-free}) that consults no cosine
or Levenshtein values, and we report Cohen's $\kappa$
\citep{cohen1960kappa,landis1977kappa} between the two labellings
as an inter-protocol stability statistic.
...
$\rho = 0.1$ small, $0.3$ medium, $0.5$ large). The median repository size is 518
steps, and the median intra-repository duplication rate is 52.5\,\%,
both markedly below the pooled 80.2\,\% rate. The pooled rate is thus
dominated by the (relatively few) very large repositories; in a
head-count-weighted average across repositories the effective rate is
approximately 27 percentage points lower.
...
\emph{both} the pooled 80.2\,\% and the median-repository 52.5\,\% are
...
A size-matched comparison would be needed for a careful causal
claim about licence, and we do not attempt it here.
...
The 1{,}020 labels used for the calibration study are author-labelled
under the written rubric, with the per-pair rule record released in
\texttt{labeled\_pairs.jsonl} (Section~\ref{sec:evaluation:labels}).
The labels themselves are not in question; the rubric's coupling
with the detector's features is. Rules R1-R3 of the primary rubric
...
52.5\,\%. Readers should compare their own codebases against the
...
$0.799$ on the same benchmark. The accompanying corpus, to our
knowledge the largest released BDD corpus
(Table~\ref{tab:corpus-compare}), comprises 347 public GitHub
repositories, approximately 23.7k parsed \texttt{.feature} files,
and 1.11M steps. Its step-weighted exact-duplicate rate is
80.2\,\% and its median-repository rate is 52.5\,\% (Spearman
