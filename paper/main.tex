...
\author{%
  ...
  \texttt{nfatima.bce25seecs@seecs.edu.pk}
  \and
  Muhammad Bilal\\
  Independent Researcher\\
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item A CDN-structured (Cognitive Dimensions of Notations)
        ...
        canonical CDN contributions typically follow;
        Appendix~\ref{app:cdn} discloses this limitation explicitly.
...
relabelling), and Appendix~\ref{app:cdn} addresses RQ4.
...
Appendix~\ref{app:cdn} presents the CDN-structured critique.
...
information artefacts: programming languages, visual notations,
forms, control panels. CDN comprises fourteen dimensions
(viscosity, hidden dependencies, premature commitment,
role-expressiveness, consistency, diffuseness, error-proneness,
hard mental operations, closeness of mapping, progressive
evaluation, provisionality, visibility, abstraction, and secondary
notation). CDN has been applied to programming languages
\citep{kutar2002cdn,clarke2004cdn}, visual programming environments
\citep{green1992visualbasic}, end-user programming
\citep{blackwell2004firstmover}, and concrete APIs
\citep{stylos2007usability,piccioni2013apis}; it has not, to our
knowledge, been applied to BDD notations in peer-reviewed
literature. We present a CDN-structured critique of Gherkin as
Appendix~\ref{app:cdn}, rating each of the fourteen dimensions
against concrete observations from our 347-repository corpus. The
headline finding that motivates the tool interventions discussed
in Section~\ref{sec:discussion} is that eight of fourteen
dimensions are rated problematic or unsupported; in particular the
viscosity and consistency ratings explain why BDD suites of
non-trivial size tend toward massive step-text duplication (our
80.2\,\% exact-duplication figure), and why standard refactoring
tools offer no help.
...
\subsection{Relationship to adjacent prior work in the same research line}
...
Comprehensive surveys of the clone-detection literature are
available in \citet{roy2007survey}, \citet{rattan2013clonesurvey},
and \citet{kim2018cloneworld}; we summarise the most directly
comparable points here. Four tools anchor the comparison space: CCFinder's token-stream
...
\paragraph{Recall = 1.000 is a stratification artefact, not a
detector property.} Before reading the F$_1$ numbers it is worth
noting that semantic's recall of 1.000 under the primary rubric
is guaranteed by the labelling-stratification construction:
every primary-rubric positive is produced by rules R1-R3, which
require cosine $\geq 0.80$, and the detector's threshold of 0.82
sits inside the same band, so no primary-rubric positive can be
missed. This pins recall at 1.000 and pulls semantic's F$_1$ upward
by definition. The discriminating numbers are precision and the
score-free F$_1$; the 0.906 primary-rubric F$_1$ for semantic is
included for completeness but is not the headline we recommend
citing.
...
different quantities and peak at different thresholds. We ran a
scout sweep on a 12-repository subsample drawn from the 347-repo
corpus by random sampling, stratified across licence class, before
the full pipeline was executed. On this scout sample we observe
the following transition in single-linkage chaining as the
Levenshtein threshold varies:
...
The 12-repository scout subsample introduced in
Section~\ref{sec:evaluation:pair-vs-cluster-role} surfaced a
striking asymmetry: within
...
Three concrete tool interventions follow from the CDN-structured
critique in Appendix~\ref{app:cdn}. The released artefact realises one of them
...
\subsection{Integration points with adjacent BDD-testing tooling}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Glue-code analysis.} Extending detection from
        ...
        Appendix~\ref{app:cdn}. AST-level diffing or neural
...
\url{https://github.com/amughalbscs16/cukereuse-release}. The corpus
...
\emph{This appendix applies the fourteen dimensions of the CDN
framework \citep{green1989cdn,green1996petre,blackwell2003cdn} to
Gherkin as a notation in production use. Each dimension is rated
as favourable, neutral, or problematic based on concrete examples
from the empirical corpus described in
...
Section~\ref{sec:corpus}; each dimension's narrative in this
appendix cites a concrete example.}
\label{fig:cdn-radar}
\end{figure}
