...
\author{%
  ...
  \and
  % TODO: add second author name + affiliation + email (labelled 420 of 1,020 pairs)
  Second Author\\
...
\begin{abstract}
Behaviour-Driven Development (BDD) suites in long-lived projects
accumulate step-text duplication whose maintenance cost is
established in prior BDD-quality work. Existing detection
techniques rely on dynamic execution tracing (Binamungu et al.,
2018-2023) or normalised compression distance at industrial scale
(Irshad et al., 2020-2022), leaving a gap: a purely
\emph{static}, \emph{paraphrase-robust}, \emph{step-level}
...
corpus of 347 public GitHub repositories, approximately 23.7k
parsed \texttt{.feature} files and 1.11M Gherkin steps. The
...
Against 1{,}020 pairs manually labelled by the authors (600 by the
first author, 420 by the second) under a released written rubric,
...
positive rules consult no cosine or Levenshtein values. The
strongest pair-level result is near-exact (Levenshtein) detection
at F$_1 = 0.822$ on the score-free labels; the semantic detector
reaches F$_1 = 0.906$ [0.886, 0.923] under the primary rubric. Two
lexical baselines standing in for classical token-stream clone
detectors (SourcererCC-style, NiCad-style) reach primary
F$_1 = 0.761$ and $0.799$. The paper also contains, to our
knowledge, the first Cognitive Dimensions of Notations analysis of
Gherkin in the peer-reviewed literature, an author assessment
grounded in corpus evidence; eight of fourteen dimensions are rated
problematic or unsupported. The tool, corpus manifest, labelled
pairs, rubric, and analysis pipeline are released under permissive
licences.
\end{abstract}
...
\paragraph{Labelling process.} The two authors labelled the 1{,}020
pairs manually under a shared written rubric
(\texttt{corpus/LABELING\_RUBRIC.md}) released alongside the corpus.
The first author labelled 600 pairs; the second author labelled the
remaining 420. Each labelling decision is a binary classification
(duplicate vs.\ not-duplicate) reflecting whether a BDD maintainer
would plausibly want the two phrasings consolidated into a single
step definition. The rubric specifies ten ordered decision rules
...
pair without re-annotating. Both authors cross-reviewed the rubric
after each 200-pair batch to resolve any ambiguous pairs and to
converge on consistent application of R4-R8 (the rules that require
light domain-specific interpretation).
